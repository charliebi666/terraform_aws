pipline {
    parameters {
        ...
        booleanParam(name: 'autoApprove', defaultValue: false, descripiton: 'Automatically run apply after generating plan?')
    }
        ...
        stage('checkout') {
            steps {
                script{
                    dir("terraform")
                    {
                        git "https://github.com/easyawslean/Terraform-Tutorial.git"
                    }
                }
            }
        }
